        ...
        <nav>
            <div class="container">
                ...
                <!--<button style="background-color: Transparent;border: none;overflow: hidden">-->
                    <a id="nav-home-tab" 
                    href=<?php echo $baseUrl."tab=1&startDate=".$settings['startDate']."&assignedUser=".$settings['assignedUser']."&sortBy=".$settings['sortBy']."&orderBy=".$settings['orderBy']."&page=1" ?>
    role="tab" aria-controls="nav-home" aria-selected="false" value="<?php echo rawurlencode(json_encode(["tab" => "1", "settings" => $settings])) ?>" <?php if ($settings["tab"] == "1") { ?> onclick="return false;"   class='nav-item nav-link border-0 active' aria-selected="true"  <?php }else{ ?> class='nav-item nav-link border-0' <?php } ?>>
                        Pending/Accepted
                    ...
                    href=<?php echo $baseUrl."tab=0&startDate=".$settings['startDate']."&assignedUser=".$settings['assignedUser']."&sortBy=".$settings['sortBy']."&orderBy=".$settings['orderBy']."&page=1" ?>
    role="tab" aria-controls="nav-profile" aria-selected="false" value="<?php echo rawurlencode(json_encode(["tab" => "0", "settings" => $settings])) ?>" <?php if ($settings["tab"] == "0") { echo "tab0"; ?> onclick="return false;"   class='nav-item nav-link border-0 active' aria-selected="true" <?php } else{ ?> class="nav-item nav-link border-0" <?php } ?>>
                        Pending/Rejected
        ...
        <div class="tab-content" id="nav-tabContent">
            ...
            <div class="pagination-bottom">
                <form method="post">
                    ...
                    <?php 
                        if ($page>1){ ?>
                            <li class="page-item"><a class="page-link chevron" 
                            href=<?php echo $baseUrl."tab=".$settings['tab']."&startDate=".$settings['startDate']."&assignedUser=".$settings['assignedUser']."&sortBy=".$settings['sortBy']."&orderBy=".$settings['orderBy']."&page=".strval($page-1)."&searchDescription=".rawurlencode($searchDescription) ?>
                        ...
                        if ($bpage > 1) {
                            ...
                            href=<?php echo $baseUrl."tab=".$settings['tab']."&startDate=".$settings['startDate']."&assignedUser=".$settings['assignedUser']."&sortBy=".$settings['sortBy']."&orderBy=".$settings['orderBy']."&page=1&searchDescription=".rawurlencode($searchDescription)?>
 
                             class="page-link" name="next" value="-1" class="viewButton">1</a></li>
                            ...
                            for (; $i < $apage; $i++) {
                                if ($i == ($page-1)) { ?>
                                <li><a 
                                href=<?php echo $baseUrl."tab=".$settings['tab']."&startDate=".$settings['startDate']."&assignedUser=".$settings['assignedUser']."&sortBy=".$settings['sortBy']."&orderBy=".$settings['orderBy']."&page=".strval($i+1)."&searchDescription=".rawurlencode($searchDescription)?>
 
                                class="page-link" style="background-color:#83c1ff;" name="next" value="<?php echo $i ?>" onclick="return false" class="viewButton"><?php echo $i + 1 ?></a></li>
                            <?php
                                } else {
                                    ?>
                                <li><a 
                                href=<?php echo $baseUrl."tab=".$settings['tab']."&startDate=".$settings['startDate']."&assignedUser=".$settings['assignedUser']."&sortBy=".$settings['sortBy']."&orderBy=".$settings['orderBy']."&page=".strval($i+1)."&searchDescription=".rawurlencode($searchDescription)?>
                                class="page-link" name="next" value="<?php echo $i - 1 ?>" class="viewButton"><?php echo $i + 1 ?></a></li>
                            <?php
                                }
                            }
                            if (($totalPages - 1 - $page) > 5) {
                                ?>
                            <li class="page-item"><a 
                            href=<?php echo $baseUrl."tab=".$settings['tab']."&startDate=".$settings['startDate']."&assignedUser=".$settings['assignedUser']."&sortBy=".$settings['sortBy']."&orderBy=".$settings['orderBy']."&page=".strval($totalPages)."&searchDescription=".rawurlencode($searchDescription) ?>
 
                             class="page-link" name="next" value="<?php echo $totalPages - 2 ?>" class="viewButton"><?php echo $totalPages ?></a></li>
                        <?php
                        }
                        ?>
